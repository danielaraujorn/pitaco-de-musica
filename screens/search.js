...
class SearchScreen extends React.Component {
  ...
  componentDidMount = () => {
    if (this.state.artist === "" && this.state.track === "") {
      if (
        ...
        this.props.navigation.state.params.artist &&
        this.props.navigation.state.params.track
      ) {
        this.setState(
          state => ({
  ...
  _search = () => {
    if (this.state.artist === "" || this.state.track === "" || this.loading)
      return;
    ...
    axios
      .get("http://ws.audioscrobbler.com/2.0/", {
        params: {
          ...
          api_key: "0c2172b42df2f27cf08619bfd7dd79b2",
          limit: 10
        }
      })
      .then(response => {
      ...
      .catch(err => {
        ...
        Alert.alert(
          "Oooops",
          "Ocorreu um erro com a solicitação, tente novamente"
        );
        this.setState({
          results: [],
          loading: false
